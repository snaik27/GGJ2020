...
public class CameraDirector : MonoBehaviour
{
    ...
    public float sphereCastRadius = 0.2f;           // the radius of the sphere used to test for object between camera and target
    ...
    public float furthestDistance = 12f;            // furthest dist cam can be from target
    public float smallestFOV = 30f;
    public float largestFOV = 80f;
    public float commonFOV = 45f;


    public bool protecting { get; set; }    // used for determining if there is an object between the target and the camera
    ...
    private void LateUpdate()
    {
        ...
        // Lerp camera distance closer or further based on m_Pivot's tilt angle 
        float normalizedPivotXAngle = (m_Pivot.eulerAngles.x < 100f) ? (m_Pivot.localEulerAngles.x + 45) / (45 + 75f) : (m_Pivot.localEulerAngles.x - 360 + 45) / (45 + 75f);
        targetDist = Mathf.Lerp(m_OriginalDist, furthestDistance, normalizedPivotXAngle);
        ...
        if (initialIntersect)
        {
            ...
            m_Hits = Physics.RaycastAll(m_Ray, furthestDistance - sphereCastRadius);
        }
        else
        {
            // if there was no collision do a sphere cast to see if there were any other collisions
            m_Hits = Physics.SphereCastAll(m_Ray, sphereCastRadius, furthestDistance + sphereCastRadius);
        }
        ...
        if (m_CurrentDist < closestDistance || m_CurrentDist > furthestDistance)
        {
            m_CurrentDist = Mathf.Clamp(m_CurrentDist, closestDistance, furthestDistance);
        }

        //normalize current dist:
        float distanceNormalized = m_CurrentDist / furthestDistance;
        m_Cam.GetComponent<Camera>().fieldOfView = Mathf.Lerp(largestFOV, smallestFOV, distanceNormalized);
